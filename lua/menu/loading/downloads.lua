...
function PANEL:PerformLayout()
	...
	for _, panel in pairs(self.Downloads) do
...
function PANEL:RefreshDownloadables()
	...
	for _, v in pairs(self.Downloadables) do
		v = string.gsub(v, "[\\/]+", "/") -- normalize slashes
		v = string.gsub(string.gsub(v, ".bz2", ""), ".ztmp", "") -- remove compression extensions

		local exists = false
		if string.find(v, ".gma$") then
			exists = file.Exists("downloads/server/" .. v, "GAME")
		else
			exists = file.Exists(v, "GAME")
		end
...
function PANEL:CurrentDownloadFinished()
	...
	if self.CurrentRunner then
		if IsValid(self.CurrentRunner) then
			self.CurrentRunner:SetShouldRepeat(false)
		end
...
function PANEL:DownloadingFile(filename)
	...
	for _, panel in pairs(self.Downloads) do
		...
		if self.CurrentRunner then
			self.CurrentRunner:SetShouldRepeat(true)
			break
		end
	end
end
